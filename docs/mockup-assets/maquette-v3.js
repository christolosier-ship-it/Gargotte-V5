...
function questDetail(q){
  ...
  return '<article class="quest-sheet-proposal family-detail-card tier-detail '+tier+'">'+familyMedia('quests',q,'quest-banner')+
    '<div class="quest-contract"><div class="quest-meta"><span style="display:flex;align-items:center;gap:6px">'+familyMark('quests','chip-logo')+tierBadge('quest',tier,q.difficulty||tierMeta[tier].quest)+'</span><span>'+(q.dungeon||'Donjon non renseigné')+'</span></div><h2>'+q.name+'</h2><div class="quest-giver">Commanditaire / PNJ : <b>'+(q.npc||'—')+'</b></div><p class="quest-story">'+(q.description||'')+'</p>'+
...
function renderQuest(){const q=sessionQuests[questIndex];$('#session-quest').innerHTML=`<div class="eyebrow" style="display:flex;align-items:center;gap:8px">${emblem('Icone_Entite_QUETE.webp','chip-logo')}<span>Quête de session</span></div><h2>${q.name}</h2><div class="row wrap"><span class="chip neutral">${q.difficulty}</span><span class="chip neutral">${q.npc}</span></div><p>${q.desc}</p><div class="quest-grid"><div class="objective"><b>Objectif</b><div class="small">${q.objective}</div></div><div class="objective"><b>Récompense</b><div class="small">${q.reward}</div></div></div>`}
