...
function show(requested){
  ...
  if(view==='codex'&&currentCodex==='creatures')applyBestiaryResponsive();
}
function initNav(){
...
function applyBestiaryResponsive(){
  const list=$('.collection'),detail=$('#creature-detail'),bar=$('.mobile-sheet-bar');if(!list||!detail||!bar)return;
  if(isPhone()){
    ...
    bar.tabIndex=0;bar.setAttribute('role','button');bar.onclick=()=>{if(mobileDetailOpen){mobileDetailOpen=false;applyBestiaryResponsive()}};
    bar.onkeydown=e=>{if(mobileDetailOpen&&(e.key==='Enter'||e.key===' ')){e.preventDefault();mobileDetailOpen=false;applyBestiaryResponsive()}};
  }else{list.style.removeProperty('display');detail.style.removeProperty('display');bar.removeAttribute('role');bar.removeAttribute('tabindex');bar.onclick=null;bar.onkeydown=null}
}
...
function initCodex(){
  ...
  addEventListener('resize',applyBestiaryResponsive);
}
